...
filename <- 'conflictChoicesAll'
# filename <- 'conflictChoicesAll_shape'
...
mdata = read_excel(path_file, "All")
...
#mdata$Moth.number <- mdata[,4]#4th column contains ID in some form
mdata$Moth.number <- mdata$AnimalID#column named animalID contains ID information
if("colourCond" %in% colnames(mdata))
{
  mdata$colourCond <- as.factor(mdata$colourCond)
}
...
# Fit model ---------------------------------------------------------------
# choices for conflict experiment
# mdata$choice1 <- mdata$Col#select what to treat as choice option 1
# mdata$choice2 <- mdata$Pat#select what to treat as choice option 2
# choiceType="_one"

# choices for non preferred - if pattern - experiment
mdata$choice1 <- mdata$nonPrefPatRight#select what to treat as choice option 1
mdata$choice2 <- 10-mdata$nonPrefPatRight#select what to treat as choice option 2
choiceType="_two"
